...
pub fn update(model: *Model, msg: Msg, fx: *Effects) void {
    ...
    switch (msg) {
        ...
        .poll_tick => |timer| {
            ...
            // selected cadence adaptive and does not redraw a settled bubble.
...
fn settleBubbleWindow(model: *Model, fx: *Effects, bubble_h: f32, want_x: f64) bool {
    ...
    if (bubbleClampCorrection(actual.x, actual.y, settled.x, settled.y)) |correction| {
        ...
        _ = recordPetCenterLocal(model, corrected.x);
    } else {
        _ = recordPetCenterLocal(model, actual.x);
    }
    return true;
}
...
fn updateBubbleScreenPlacement(model: *Model) void {
    ...
    const pet_top_y = bubblePetTopY(model);
    const pet_bottom_y = pet_top_y + pet_h;
    const screen = plat.visibleScreenFrameAt(pet_center_x, pet_center_y) orelse {
        model.bubble_secondary_lane = false;
        model.bubble_flipped = bubbleShouldFlip(model, pet_top_y, @floatCast(bubbleCurrentWindowHeight(model)));
        return;
    };
    ...
    const above = @max(0, pet_top_y - screen.y);
    const below = @max(0, screen.y + screen.height - pet_bottom_y);
...
fn syncBubbleWindow(model: *Model, fx: *Effects) void {
    ...
    if (bubbleMovePlan(cur.x, cur.y, want_x, want_y)) |plan| {
        ...
        actual = fx.moveWindow("bubble", plan.dx, plan.dy, false) orelse return;
        model.bubble_window_presentation_generation +%= 1;
        invalidateBubblePresentation(model, .{ .geometry = true });
    }
    // Always probe the destination display, including a zero-distance target:
    // taskbar, display-layout, and display-scale changes can invalidate an
    // otherwise stationary placement. The unbounded leg above must remain
    // first so a bubble can cross displays before that target clamps it.
    const settled = fx.moveWindow("bubble", 0, 0, true) orelse return;
    actual = fx.moveWindow("bubble", 0, 0, false) orelse return;
    if (bubbleClampCorrection(actual.x, actual.y, settled.x, settled.y)) |correction| {
        actual = fx.moveWindow("bubble", correction.dx, correction.dy, false) orelse return;
    }
...
test "bubble movement crosses displays before applying target bounds" {
    ...
    const sync_end = std.mem.indexOfPos(u8, src, sync_start, "fn applyBubbleNativeControlSemantics").?;
    const sync = src[sync_start..sync_end];
    ...
    const no_move_boundary = std.mem.indexOf(u8, sync, "// Always probe the destination display");
    const bounded = std.mem.indexOf(u8, sync, "fx.moveWindow(\"bubble\", 0, 0, true)");
    const readback = std.mem.indexOf(u8, sync, "actual = fx.moveWindow(\"bubble\", 0, 0, false)");
    ...
    try std.testing.expect(unbounded.? < no_move_boundary.?);
    try std.testing.expect(no_move_boundary.? < bounded.?);
