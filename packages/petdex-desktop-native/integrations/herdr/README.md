...
Herdr starts one bridge process per status event. Near-simultaneous events use last-write-wins aggregation, so a newer state can briefly be replaced by an older snapshot.
