...
import { type Agent, AGENTS, PETDEX_PORT, SIDECAR_URL } from "./agents.js";
	...
	if (status.state === "running") {
		...
		if (!p.isCancel(shouldStart) && shouldStart) {
			...
			if (result.ok) {
				...
				p.log.warn(`${pc.yellow("!")} Could not start petdex-desktop: ${result.reason}`);
				p.log.info(
...
async function installForAgent(agent: Agent): Promise<InstallResult> {
	...
	const backupPath = existing != null ? await maybeBackup(agent.configFile) : null;
...
function isPetdexEntry(entry: unknown): boolean {
	...
	return cmds.some((c) => c.includes(`localhost:${PETDEX_PORT}/state`) || c.includes(SIDECAR_URL));
}
